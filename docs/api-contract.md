...
| `POST /api/reports/build` | Phase 7 additive：→ `DesignReportPayload`（JSON）；前端不重算面积/评分；**dirty 默认 409** `candidate_requires_revalidation`；**candidate_id / selected_id 指定但找不到 → 404** `candidate_not_found`；**placement 缺 area → 400** `placement_area_missing`（禁止 width×depth 猜测）；详见 [phase-7-deliverables.md](phase-7-deliverables.md) |
